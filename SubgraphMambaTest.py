...
def buildModel(hidden_dim, conv_layer, dropout, jk, pool, z_ratio, aggr):
    '''
    Build a SubgraphMamba model.
    Args:
        jk: whether to use Jumping Knowledge Network.
        conv_layer: number of SubgraphMambaConv.
        pool: pooling function transfer node embeddings to subgraph embeddings.
        z_ratio: see SubgraphMambaConv in impl/model.py. Z_ratio in [0.5, 1].
    ...
    conv = models.EmbZGConv(hidden_dim,
                            ...
                            jk=jk,
                            dropout=dropout,
                            conv=functools.partial(models.SubgraphMambaConv,
                                                   aggr=aggr,
                                                   z_ratio=z_ratio,
                                                   dropout=dropout),
                            gn=True)
    ...
    gnn = models.SubgraphMamba(conv, torch.nn.ModuleList([mlp]),
...
def test(pool="size",
    ...
    Args:
        jk: whether to use Jumping Knowledge Network.
        z_ratio: see SubgraphMambaConv in impl/model.py. A hyperparameter of SubgraphMamba.
